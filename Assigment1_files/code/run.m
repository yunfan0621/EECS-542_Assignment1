...
lower_arm_r = struct('name',   'lower_arm_r',...
                     'part_id', 5,...
                     'B',       [],...
                     'Bj_p',    []); % leaf node

lower_arm_l = struct('name',   'lower_arm_l',...
                     'part_id', 4,...
                     'B',       [],...
                     'Bj_p',    []); % leaf node
...
lower_arm_r.B = zeros(size(torso.B));
lower_arm_l.B = zeros(size(torso.B));
...
lower_arm_r.Bj_p = cell(size(torso.B));
lower_arm_l.Bj_p = cell(size(torso.B));
...
T_lower_arm_l_upper_arm_l = calc_Tij(lower_arm_l.part_id, upper_arm_l.part_id, x_grid, y_grid, theta_grid, s_grid, opt);
T_lower_arm_r_upper_arm_r = calc_Tij(lower_arm_r.part_id, upper_arm_r.part_id, x_grid, y_grid, theta_grid, s_grid, opt);
...
for l_ind = 1 : n_search 
    ...
    lower_arm_r_neighbors = Inf(1, 5);
    lower_arm_l_neighbors = Inf(1, 5);

    head_neighbors_lj = zeros(5, 4); % for argmin value, 5-neighbors
    lower_arm_r_neighbors_lj = zeros(5, 4);
    lower_arm_l_neighbors_lj = zeros(5, 4);
    ...
    Tji = T_lower_arm_r_upper_arm_r(:, Lj);
    Tij = T_upper_arm_r_lower_arm_r(:, Li);
    dij = sum(abs(Tij - Tji));
    lower_arm_r_neighbors(1)  = dij + lower_arm_r.B(Lj);
    lower_arm_r_neighbors_lj(1, :)  = lower_arm_r.Bj_p{Lj};
    
    Tji = T_lower_arm_l_upper_arm_l(:, Lj);
    Tij = T_upper_arm_l_lower_arm_l(:, Li);
    dij = sum(abs(Tij - Tji));
    lower_arm_l_neighbors(1)  = dij + lower_arm_l.B(Lj);
    lower_arm_l_neighbors_lj(1, :)  = lower_arm_l.Bj_p{Lj};
    ...
    if (x_ind-1 >= 1)
        ...
        Tji = T_lower_arm_r_upper_arm_r(:, Lj_x);
        Tij = T_upper_arm_r_lower_arm_r(:, Li);
        dij = sum(abs(Tij - Tji));
        lower_arm_r_neighbors(2)  = dij + lower_arm_r.B(Lj_x);
        lower_arm_r_neighbors_lj(2, :)  = lower_arm_r.Bj_p{Lj_x};

        Tji = T_lower_arm_l_upper_arm_l(:, Lj_x);
        Tij = T_upper_arm_l_lower_arm_l(:, Li);
        dij = sum(abs(Tij - Tji));
        lower_arm_l_neighbors(2)  = dij + lower_arm_l.B(Lj_x);
        lower_arm_l_neighbors_lj(2, :)  = lower_arm_l.Bj_p{Lj_x};
    end
    ...
    if (y_ind-1 >= 1) 
        ...
        Tji = T_lower_arm_r_upper_arm_r(:, Lj_y);
        Tij = T_upper_arm_r_lower_arm_r(:, Li);
        dij = sum(abs(Tij - Tji));
        lower_arm_r_neighbors(3)  = dij + lower_arm_r.B(Lj_y);
        lower_arm_r_neighbors_lj(3, :)  = lower_arm_r.Bj_p{Lj_y};

        Tji = T_lower_arm_l_upper_arm_l(:, Lj_y);
        Tij = T_upper_arm_l_lower_arm_l(:, Li);
        dij = sum(abs(Tij - Tji));
        lower_arm_l_neighbors(3)  = dij + lower_arm_l.B(Lj_y);
        lower_arm_l_neighbors_lj(3, :)  = lower_arm_l.Bj_p{Lj_y};
    end
    ...
    if (theta_ind-1 >= 1)
        ...
        Tji = T_lower_arm_r_upper_arm_r(:, Lj_theta);
        Tij = T_upper_arm_r_lower_arm_r(:, Li);
        dij = sum(abs(Tij - Tji));
        lower_arm_r_neighbors(4)  = dij + lower_arm_r.B(Lj_theta);
        lower_arm_r_neighbors_lj(4, :)  = lower_arm_r.Bj_p{Lj_theta};

        Tji = T_lower_arm_l_upper_arm_l(:, Lj_theta);
        Tij = T_upper_arm_l_lower_arm_l(:, Li);
        dij = sum(abs(Tij - Tji));
        lower_arm_l_neighbors(4)  = dij + lower_arm_l.B(Lj_theta);
        lower_arm_l_neighbors_lj(4, :)  = lower_arm_l.Bj_p{Lj_theta};
    end
    ...
    if (s_ind-1 >= 1) 
        ...
        Tji = T_lower_arm_r_upper_arm_r(:, Lj_s);
        Tij = T_upper_arm_r_lower_arm_r(:, Li);
        dij = sum(abs(Tij - Tji));
        lower_arm_r_neighbors(5)  = dij + lower_arm_r.B(Lj_s);
        lower_arm_r_neighbors_lj(5, :)  = lower_arm_r.Bj_p{Lj_s};

        Tji = T_lower_arm_l_upper_arm_l(:, Lj_s);
        Tij = T_upper_arm_l_lower_arm_l(:, Li);
        dij = sum(abs(Tij - Tji));
        lower_arm_l_neighbors(5)  = dij + lower_arm_l.B(Lj_s);
        lower_arm_l_neighbors_lj(5, :)  = lower_arm_l.Bj_p{Lj_s};
    end

    % obtain the min/argmin value
    [head_min_val, head_min_ind] = min(head_neighbors);
    [lower_arm_l_min_val, lower_arm_l_min_ind] = min(lower_arm_l_neighbors);
    [lower_arm_r_min_val, lower_arm_r_min_ind] = min(lower_arm_r_neighbors);
    
    head.B(Li) = head_min_val;
    lower_arm_l.B(Li) = lower_arm_l_min_val;
    lower_arm_r.B(Li) = lower_arm_r_min_val;
    
    head.Bj_p{Li} = head_neighbors_lj(head_min_ind, :);
    lower_arm_l.Bj_p{Li} = lower_arm_l_neighbors_lj(lower_arm_l_min_ind, :);
...
fprintf('\nForward pass through D for all intermidal nodes...\n');
for l_ind = 1 : n_search 
    % search for all possibilities of Li, increase (x,y,theta,s) in order
    ...
    % initialize neighbor vectors
    upper_arm_r_neighbors = Inf(1, 5); % for min value
    upper_arm_l_neighbors = Inf(1, 5);
    
    upper_arm_r_neighbors_lj = zeros(5, 4); % for argmin value, 5-neighbors
...
%% Compute minimum distance in D for intermidal nodes (Backward Pass)
